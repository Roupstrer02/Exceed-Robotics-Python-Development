...
while True:
    ...
    for mole in moles:
        if mole.collidepoint(mousePos) and justClicked:
            moleColours.remove(moleColours[moles.index(mole)])
            moles.remove(mole)
            moleTimer -= 1
            score += 1
            break
    ...
    if moleCooldown == 0:
        ...
        moleColours.append((randint(50,255),randint(20,255),randint(20,255)))
    else:
        moleCooldown -= 1
    ...
    for i in range(0,len(moles)):
        ...
        pg.draw.rect(screen, moleColours[i], moles[i])
